...
public class HelloController extends ParentController {
    ...
    @GetMapping("/say")
    public List<Company> say() {
        return entityQuery.queryable(Company.class).where(c -> c.name().like("小米"))
                .toList();
    }
    ...
    // 使用 ds2 数据源 EasyQuery 查询
    @GetMapping("/ds2/say")
    public List<Company> ds2Say() {
        return ds2EntityQuery.queryable(Company.class).where(c -> c.name().like("小米"))
                .toList();
    }
}
